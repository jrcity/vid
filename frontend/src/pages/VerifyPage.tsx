...
const VerifyPage: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        <SEO title={t('verification.verifying')} />
        <div className="w-12 h-12 border-4 border-slate-200 border-t-brand-accent rounded-full animate-spin" />
        <p className="text-slate-500 font-medium animate-pulse">{t('verification.verifying')}</p>
      </div>
    )
  }
  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
        <SEO title={t('verification.failed_title')} />
        <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center text-4xl">
          <MdErrorOutline />
        </div>
        <h2 className="text-xl font-bold text-brand-dark">{t('verification.failed_title')}</h2>
        <p className="text-slate-500 max-w-xs">{t('verification.failed_desc')}</p>
        <Link to="/" className="native-button bg-slate-100 px-8 text-slate-600 mt-4">{t('verification.go_back')}</Link>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      <SEO title={t('verification.title')} description={`Public verification for VID ${data.vid_id}. Status: ${data.trust_grade}.`} />
      ...
      <header className="text-center">
        ...
        <h1 className="text-2xl font-bold text-brand-dark">{t('verification.title')}</h1>
        <p className="text-slate-500 text-sm">{t('verification.subtitle')}</p>
      </header>
      ...
      <section className="space-y-4">
        ...
        >
          ...
          <div className="flex justify-between items-start z-10">
            <div className="space-y-1">
              ...
              <h2 className="text-3xl font-black tracking-tight text-brand-dark/50 italic">{t('verification.confidential_holder')}</h2>
            </div>
          ...
          <div className="z-10 flex items-end justify-between">
            <div className="space-y-2">
              ...
              <div className="flex gap-8">
                <div>
                  <p className="text-[9px] uppercase tracking-[0.2em] text-brand-dark/50 font-bold">{t('verification.issued')}</p>
        ...
        <div className={`native-card p-6 space-y-4 ${scoreBg}`}>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-[8px] uppercase text-slate-400">{t('verification.nationality')}</p>
            ...
            <div>
              <p className="text-[8px] uppercase text-slate-400">{t('verification.trust_score')}</p>
              <p className={clsx("text-sm font-bold", scoreColor)}>{data.score}/100</p>
            </div>
            <div>
              <p className="text-[8px] uppercase text-slate-400">{t('verification.expires')}</p>
        ...
        <div className="native-card p-5 flex gap-4 items-center">
          ...
          <div>
            <p className="text-[10px] uppercase text-slate-400">{t('verification.issuing_authority')}</p>
            <p className="text-xs font-bold text-brand-dark">{t('verification.authority_name')}</p>
          </div>
        </div>
      </section>
